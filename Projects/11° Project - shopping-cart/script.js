...
function setCounter() {
  ...
  document.querySelector('.span-counter').innerHTML = listItems.length;
}
...
async function putPrice() {
  ...
  try {
    ...
    if (price === 0) elementPrice.innerText = '0,00';
    else elementPrice.innerText = `${price.toFixed(2)}`;
  } catch (error) {
    window.alert(error);
  }
}
...
function addEventItemInMyCart(element, id) {
